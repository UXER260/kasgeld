...
with open(config["accounts_path"]) as f:
    all_account_data = json.load(f)
...
def get_account_data(account_name):
    account = all_account_data.get(account_name)
    if account is None:
...
def delete_account(account_name):
    try:
        del all_account_data[account_name]
        apply_changes()
        return True  # todo
    except KeyError:
        return False  # todo
...
def rename_account(account_name: str, new_name: str):
    if not check_account_exists(new_name):
        all_account_data[account_name]["name"] = new_name
        all_account_data[new_name] = all_account_data.pop(account_name)
        apply_changes()
    else:
        return "ExistsError"
...
def check_account_exists(account_name: str):
    ...
    return account_name in all_account_data.keys()
...
def add_account_to_file(account_info: AccountField):  # obvious
    ...
    t_and_d = time_and_date()
    all_account_data[name] = {"name": name, "money": saldo, "transactions": transactions, "savings": savings,
                              "last_salary_date": [t_and_d.year, t_and_d.month]}
    apply_changes()
    return all_account_data[name]


@app.get("/get_account_name_list")
def get_account_name_list() -> list:  # obvious
    return [account["name"] for account in all_account_data.values()]


@app.put("/apply_changes")
def apply_changes(account_data=None) -> None:  # writes changes to "accounts" file
    with open(config["accounts_path"], "w") as file:
        json.dump(account_data if account_data is not None else all_account_data, file, indent=2)
...
def set_saldo(account: AccountField, transaction_details: TransactionField):
    if not check_account_exists(account["name"]):
        print("Account does not exist")
        return False  # todo
    all_account_data[account["name"]]["transactions"].append(
    ...
    all_account_data[account["name"]]["money"] = transaction_details.amount_to_set

    account["money"] = all_account_data[account["name"]]["money"]
    account["transactions"] = all_account_data[account["name"]]["transactions"]

    apply_changes()  # writes to the actual accounts file.
    return account
...
def get_transaction_header_list(account_name: str):
    ...
    for transaction in all_account_data[account_name]["transactions"]:

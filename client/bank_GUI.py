...
def select(window=None):
    with open("kasgeld.txt") as f:
        leerlingen = [{"naam": naam, "geld": float(geld)} for naam, geld in
                      [line.split(" ") for line in [line.strip() for line in f.readlines()]]]
        # leerlingen = [line.strip() for line in f.readlines()]
        # leerlingen = [line.split(" ") for line in leerlingen]
        # leerlingen = [{"naam": naam, "geld": geld} for naam, geld in leerlingen]
    ...
    for leerling in leerlingen:
        first_transaction = Account.generate_transaction(
            ...
            transaction_details=TransactionField(
                amount_to_set=leerling["geld"],
                title="start bedrag",
                description=f"Start bedrag van {leerling['naam']}",
                date="",
                time=""
            ))
        account = AccountField(name=leerling["naam"], money=leerling["geld"], transactions=[first_transaction],
                               savings=[])
        Account.add_account_to_file(account_info=account)
